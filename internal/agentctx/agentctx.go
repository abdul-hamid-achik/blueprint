...
func blueprintMeta(version string) BlueprintMeta {
	return BlueprintMeta{
		Version: version,
		Summary: "Blueprint is a Go-based compiler. `.bp` DSL sources describe data models, business pipelines, HTTP endpoints, middleware, and streaming handlers; `bp build` emits a runnable project in TypeScript+Hono+Drizzle (default) or Python+FastAPI+SQLAlchemy (`--target python`).",
	}
}
...
func commandRegistry() []CommandInfo {
	return []CommandInfo{
		...
		{"build", "bp build <file.bp> [--out <dir>] [--target node|python] [--gen-tests]", "Compile to a runnable project (idempotent)."},
		...
		{"context", "bp context [topic] [--format md|json]", "This command. Agent-facing language + CLI surface."},
