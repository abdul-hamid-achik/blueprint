...
func exprToJSWithCtx(e ast.Expr, ctx *emitCtx) string {
	...
	case *ast.FnCall:
		...
		case "join":
			// WS built-in: join room(id) — room management placeholder
			if len(v.Args) >= 1 {
				arg := resolveWsRoomArg(v.Args[0], ctx)
				return fmt.Sprintf("/* TODO: join room %s */", arg)
			}
			return "/* TODO: join room */"
		case "leave":
			// WS built-in: leave room(id) — room management placeholder
			if len(v.Args) >= 1 {
				arg := resolveWsRoomArg(v.Args[0], ctx)
				return fmt.Sprintf("/* TODO: leave room %s */", arg)
			}
			return "/* TODO: leave room */"
		case "broadcast":
			// WS built-in: broadcast room(id) { data }
			if len(v.Args) >= 2 {
				arg := resolveWsRoomArg(v.Args[0], ctx)
				data := exprToJSWithCtx(v.Args[1], ctx)
				return fmt.Sprintf("/* TODO: broadcast to room %s: %s */", arg, data)
			}
			if len(v.Args) >= 1 {
				arg := resolveWsRoomArg(v.Args[0], ctx)
				return fmt.Sprintf("/* TODO: broadcast to room %s */", arg)
			}
