...
func exprToJSWithCtx(e ast.Expr, ctx *emitCtx) string {
	...
	case *ast.FnCall:
		...
		case "join":
			...
			// In WS context, 'ws' is available from the Hono WebSocket handler
			if len(v.Args) >= 1 {
				arg := resolveWsRoomArg(v.Args[0], ctx)
				// _rooms is a module-level Map<roomId, Set<ws>> generated by genWsRoute
				return fmt.Sprintf("(_rooms.get(%s) ?? _rooms.set(%s, new Set()).get(%s))!.add(ws)", arg, arg, arg)
			}
		...
		case "leave":
			...
			if len(v.Args) >= 1 {
				arg := resolveWsRoomArg(v.Args[0], ctx)
				return fmt.Sprintf("_rooms.get(%s)?.delete(ws)", arg)
			}
