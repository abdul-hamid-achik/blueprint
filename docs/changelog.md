...
- **New CLI commands:**
  - `bp diff` — Preview changes before building
  - `bp deploy` — Deploy to Docker or Fly.io
  - `bp stats` — Show code statistics for .bp files
  - `bp doctor` — Check environment dependencies
  - `bp completion <bash|zsh|fish>` — Shell completion scripts
  - `bp lsp` — Language Server Protocol server
  - `bp check --json` — JSON output for CI integration
- **LSP Server** (`internal/lsp/server.go`): Basic LSP implementation with initialize, hover, and document sync support
- **Package Registry** (`internal/registry/registry.go`): Foundation for `bp add/install` package management
- **"Did you mean?" suggestions**: Command typo detection with helpful suggestions

### Changed

- Improved CLI help text with consistent formatting
- Added JSON output option to `bp check` for machine-readable error reporting
